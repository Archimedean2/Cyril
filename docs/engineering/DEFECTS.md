...
| D-25 | 🟠 | **The rhyme list became hard to skim.** C-43 added a secondary copy control rendered in the normal layout flow at `opacity: 0`, so it reserved width beside every result; each result also carried its own comma element. Gaps varied word to word, commas orphaned themselves at the start of wrapped lines, and the literal word "copy" appeared once per result down the rail. Scanning forty rhymes is the pane's whole job. | No test asserted anything about layout rhythm, and jsdom runs no layout. Reported by the maintainer looking at it. | `T-14.23` |
| D-26 | 🟠 | **A rail control stole focus from the lyric, and the browser's own undo then ate the document.** Clicking an Inventory chip to insert its word (C-42) left focus on the chip button: a button takes focus on mousedown, before the click handler runs, and Tiptap's `focus()` inside the insert did not reliably win it back. The writer's next keystroke went nowhere — and `Cmd+Z`, now aimed at the browser's native undo rather than ProseMirror's, emptied the editor. | jsdom has no focus model and no native undo, so the integration test inserted the word and passed. Only a real browser can see it. | `T-14.30` (asserts focus is back in the editor *and* that typing continues in the lyric) — fixed with `onMouseDown` → `preventDefault()` |
| D-27 | 🟠 | **An inserted word undid together with the sentence before it.** `insertAtCaret` dispatched one transaction, which is one undo *step* — but prosemirror-history folds adjacent steps into one undo *event* when they fall within its 500 ms group delay. A chip clicked shortly after typing therefore joined the typing group, and one `Cmd+Z` removed both the inserted word and the line the writer had just typed. | "One transaction" was tested (`T-4.40`) and is true; "one undo event" is the thing a writer actually experiences, and nothing asserted it against a document with prior history. | `T-14.31` (browser + unit) — fixed by calling `closeHistory` on the transaction before inserting |
| D-24 | 🟠 | **The Tools rail's "populate from selection" control was inert.** `RightSidebar.tsx` defined `getSelectedText = () => null` behind a TODO, so the ⌖ button rendered (the prop is a function, hence truthy) and did nothing when clicked. A visible control that silently does nothing is worse than no control. | Nothing asserted that a rendered control has an effect. | `T-14.29` — C-41 removes the control outright and replaces the gesture with a double-click in the lyric |
| D-28 | 🔴 | **A chord does not follow its letter when the lyric is edited.** Typing before a chord's anchor leaves the stored `charOffset` untouched, so the chord now sits over the wrong word — silently, while the writer types. Reproduced 2026-09-12: a chord at offset 8 of "nothing left", caret to the start, type "oh " (3 chars) → the chord is still at offset 8, now over "t left" instead of "left". `EDGE_CASES.md` §1 lists this 🔴 and it had never been tested. | `chords.lyric-edit-safety.test.ts` asserted only that the offset stays a finite number ≥ 0 — which it does, wrongly. Asserting a value is *valid* is not asserting it is *right*. | Open — `BACKLOG.md` C-60 |
| D-29 | 🔴 | **Splitting a chorded line duplicates every chord onto both halves.** Pressing Enter mid-line copies the node's attrs to the new node, chords included, so both lines end up carrying the whole original set at their original offsets. Reproduced 2026-09-12: "nothing left" with C@0 and G@8, split at offset 8 → both resulting lines hold `[C@0, G@8]`. Each chord should follow its own text. | Same gap as D-28: nothing exercised chord data across a structural edit. | Open — `BACKLOG.md` C-60 |
