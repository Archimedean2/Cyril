...
1. `docs/archive/STAGES.md` — Section `## Stage 12: Lightweight Sharing` (line ~819). Master stage definition.
2. `docs/product/FEATURES.md` — Section `## Feature 12: Lightweight Sharing` (line ~812). Feature spec. Note excluded behaviors.
3. `docs/product/SCOPE.md` — Search for "share", "collaboration", "offline" to find scope limits.
4. `docs/engineering/ARCHITECTURE.md` — Search for "local-first", "export", "share" to understand where sharing fits.
5. `docs/engineering/DATA_MODEL.md` — Search for "share", "export" to see schema requirements.
6. `docs/archive/PROGRESS.md` — Verify Stage 11 is Complete before proceeding.
...
Path A — Skip: Mark Stage 12 `Deferred` in docs/archive/PROGRESS.md, update the test spec with a note, and stop.
...
Path D — Backend Share: Only if docs/engineering/ARCHITECTURE.md explicitly describes a backend or sync service. Cite the decision. Do not invent one.
...
- Update `docs/archive/PROGRESS.md` and `tests/specs/stage-12.md`.
...
- docs/archive/PROGRESS.md and stage-12.md updated.
