...
**Do:** Configure vitest with `c8` or `istanbul` coverage. Add a `test:coverage` script to `package.json`. Run it once and record the baseline numbers in `docs/archive/PROGRESS.md`.

**Measurable outcome:**
- [x] `npm run test:coverage` succeeds and prints a coverage summary
- [x] `docs/archive/PROGRESS.md` coverage table has real numbers for: domain, persistence, editor, export, UI, overall
