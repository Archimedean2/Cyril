...
- Phase 3 feel layer (Stage 15) **complete**: focus mode (Cmd+\, T-15.01/02); empty draft placeholder via custom ProseMirror plugin (T-15.03); LeftNav + root app error boundary coverage (T-15.04/05); Cmd+[ / Cmd+] draft-switching shortcuts (T-15.06/07); Cmd+K command menu with shortcut reference (T-15.08/09). Feature coverage 100% (135/135 non-e2e criteria).
- Concurrent-speaker row markers (Enhancement E1) **complete**: ProseMirror plugin adds `concurrent-block--focused` to the block and `lyric-line--active-row` to the active row across all columns when the caret is inside the block; both classes are removed when focus leaves. Pure CSS — no schema or export change. T-13.16a and T-13.16b passing.
- Stress-mark row alignment bug fix **complete**: CSS `:has()` block-level rule equalises line-height across all columns when any line in the block carries `.cyril-stress-mark` (line-height 2.2) or `.cyril-chord-anchor` (line-height 2.6) elements, keeping rows aligned even when one column has stress marks and another doesn't. T-13.17 passing. Feature coverage 100% (136/136 non-e2e criteria).
...
Copy the acceptance criteria for the current stage from `docs/archive/STAGES.md` and track them here.
...
- Files modified: `src/editor/core/draftConfig.ts`, `src/components/editor/DraftToolbar.tsx`, `src/domain/project/types.ts`, `src/domain/project/defaults.ts`, `src/domain/project/migration.ts`, `src/domain/export/exportTypes.ts`, `src/domain/export/exportSelectors.ts`, `src/domain/export/exportService.ts`, `src/domain/export/concurrentExport.ts`, `src/domain/export/markdownTransformer.ts`, `src/domain/export/printRenderer.ts`, `docs/process/DOC_MAP.md`
- Deviations: None
