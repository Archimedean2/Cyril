...
| T-1.32 | An autosave and a manual save racing on the same handle never interleave — the save invoked last always determines the final on-disk content | unit | `tests/unit/persistence/fileManager.test.ts` | [x] | [x] | EDGE_CASES §8 🟠 (C-31 hardening follow-up); fixed a genuine race — saveProject calls are now serialized through a queue |
| T-1.33 | A file removed from disk after Open causes the next save (manual or autosave) to reject loudly, never to silently report success or lose the edit | unit | `tests/unit/persistence/fileManager.test.ts` | [x] | [x] | EDGE_CASES §8 🔴 "file moved/deleted/renamed externally" |
| T-1.34 | Recovery snapshot writes degrade gracefully (resolve false/null) when IndexedDB.open fails, e.g. quota exceeded/private browsing | unit | `tests/unit/persistence/recovery-store.test.ts` | [x] | [x] | EDGE_CASES §8 🟠 "IndexedDB unavailable/quota-exceeded"; existing test, retagged with an ID |
| T-1.35 | The tool cache store degrades to no-ops/empty results (not rejections) when IndexedDB.open fails | unit | `tests/unit/persistence/tool-cache-store.test.ts` | [x] | [x] | EDGE_CASES §8 🟠 "...tool cache must degrade, no crash"; fixed a genuine bug — every method previously rejected, with no try/catch in the consumer |
