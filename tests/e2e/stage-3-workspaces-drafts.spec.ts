...
test.describe('Stage 3: Workspaces and Drafts', () => {
  ...
  test('T-3.10: Workspace/draft flow passes in UI', async ({ page }) => {
    ...
    // Type in Brief workspace — use ProseMirror locator to avoid matching the tools search input
    const editor = page.locator('[data-testid="editor-surface"] .ProseMirror, main .ProseMirror').first();
    await editor.click();
